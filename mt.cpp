...
public:
    ...
    Olimpyc(const Olimpyc* ol) {
        strcpy(country, ol->country);
        for (int i = 0; i < 3; i++) {
            medals[i] = ol->medals ? ol->medals[i] : 0;
        }
    }
    void print() const {
        cout << country << " - [";
...
public:
    MedalkiTab(int umax = 10) :max{ umax }, current{ 0 } {
        table = new Olimpyc[umax];
    };
    MedalkiTab(const MedalkiTab* a) {
        MedalkiTab(a->max);
    }
    void print() const {
        for (int i = 0; i < current; i++) {
    ...
    MedalkiTab& operator=(const MedalkiTab* mt1) {
        table = mt1->table;
        max = mt1->max;
        current = mt1->current;
        return *this;
    }
    friend ostream& operator<<(ostream& out, const MedalkiTab& m1);
    friend istream& operator>>(istream& in, MedalkiTab& m1);
};

ostream& operator<<(ostream& out, const MedalkiTab& m1) {
    for (int i = 0; i < m1.current; i++) {
        out << i << ": ";
        m1.table[i].print();
    }
    return out;
...
istream& operator>>(istream& in, MedalkiTab& m1) {
    in >> m1.max;
    return in;
}
